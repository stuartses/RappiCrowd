...
class LoginSolicitante extends Component {
	...
	SubmitHandler = e => {
		...
		const passwordDigitado = this.state.workerPassword;
		const personaEncontradaw = this.props.workers.filter(personaw => {
			if ((personaw.workerCed === cedulaDigitada) && (personaw.workerPassword === passwordDigitado))  {
		          return personaw;     
		        }
		});
		this.props.history.push(`/solicitante/${personaEncontradaw[0].workerCed}`);
	};
